// https://github.com/mantinedev/mantine/blob/master/packages/%40mantine/hooks/src/use-local-storage/use-local-storage.ts
...
function serializeJSON<T>(
  value: T,
  hookName: string = "use-local-storage-repo"
) {
  try {
...
function createStorage<T>(hookName: string) {
...
function useLocalStorage<T = string>(props: StorageProperties<T>) {
  return createStorage<T>("use-local-storage")(props);
}
