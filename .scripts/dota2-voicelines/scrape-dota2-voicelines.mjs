...
//
// Usage:
//   node .scripts/dota2-voicelines/scrape-dota2-voicelines.mjs                  # all heroes
//   node .scripts/dota2-voicelines/scrape-dota2-voicelines.mjs --heroes "Axe,Pudge"
//   node .scripts/dota2-voicelines/scrape-dota2-voicelines.mjs --heroes Axe --upload
//   node .scripts/dota2-voicelines/scrape-dota2-voicelines.mjs --dry-run        # metadata only
//   node .scripts/dota2-voicelines/scrape-dota2-voicelines.mjs --refresh        # ignore cache
...
/**
 * One hero entry in the metadata index.
 ...
 * @param {string} hero
 * @param {string} slug
 * @param {{ refresh: boolean, parseDelayMs: number }} options
 * @returns {Promise<{ html: string, cached: boolean }>}
 */
async function fetchResponsesHtml(hero, slug, { refresh, parseDelayMs }) {
  ...
  const json =
    /** @type {{ error?: { code: string, info: string }, parse?: { text?: Record<string, string> } }} */ (
      await res.json()
    );
...
/**
 * Download one mp3 to disk unless it already exists.
 * @param {import("./lib/dota2-responses.mjs").Voiceline} line
 * @param {string} slug
 * @returns {Promise<{ status: "downloaded", bytes: number } | { status: "skipped" } | { status: "failed", error: string }>}
 */
async function downloadMp3(line, slug) {
  const dest = path.join(AUDIO_DIR, slug, line.filename);
  if (await exists(dest)) return { status: "skipped" };
...
/**
 * Upload one object to R2, skipping the PUT if it already exists.
 * @param {import("@aws-sdk/client-s3").S3Client} s3
 * @param {string} bucket
 * @param {string} key
 * @param {Buffer | Uint8Array} body
 * @param {boolean} refresh
 * @returns {Promise<"exists" | "uploaded">}
 */
async function uploadToR2(s3, bucket, key, body, refresh) {
  if (!refresh) {
    try {
      ...
      const e =
        /** @type {{ $metadata?: { httpStatusCode?: number }, name?: string }} */ (
          err
        );
      const code = e.$metadata?.httpStatusCode;
      if (code !== 404 && e.name !== "NotFound") throw err;
    }
  }
  await s3.send(
 ...
 * @returns {Promise<Record<string, IndexEntry>>}
 */
async function readIndex() {
  const p = path.join(META_DIR, "index.json");
  if (!(await exists(p))) return {};
  try {
    /** @type {{ heroes?: IndexEntry[] }} */
...
export async function main() {
  ...
  program
    ...
    .parse();
  const opts =
    /** @type {{ heroes?: string, upload: boolean, dryRun: boolean, refresh: boolean, parseDelay: string, concurrency: string, limit?: string }} */ (
      program.opts()
    );
  ...
  if (opts.upload) {
    ...
    // Env presence is validated above, so assert the config shape.
    s3 = new S3Client(
  ...
  for (const hero of heroes) {
    ...
    try {
        ...
        await mapPool(lines, concurrency, async (line, i) => {
          ...
          if (s3) {
            ...
            const r = await uploadToR2(
              s3,
              // Validated present when `s3` (and thus upload) is configured.
              /** @type {string} */ (bucket),
      ...
      problems.push(`${hero}: ${message}`);
      spin.fail(`${chalk.yellow(hero)}  ${chalk.red(message)}`);
    }
  }
