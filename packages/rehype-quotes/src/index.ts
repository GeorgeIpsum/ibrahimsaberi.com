...
function lastElement(node: Element): Element | undefined {
...
function startsWithMarker(value: string, marker: string): boolean {
...
function stripMarker(node: Text, marker: string): void {
...
function wrapCiteLinks(figcaption: Element): void {
  visit(figcaption, "element", (link, index, parent) => {
    if (link.tagName !== "a" || typeof index !== "number" || !parent) return;
    const text = leadingText(link);
    if (!text || !startsWithMarker(text.value, CITE_MARKER)) return;
...
export default function rehypeQuotes(): (tree: Root) => void {
  return (tree) => {
    visit(tree, "element", (node, index, parent) => {
      ...
      let citation: Element | undefined;
