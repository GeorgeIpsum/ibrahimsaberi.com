...
function loneImage(node: Element): Element | undefined {
...
export default function rehypeFigcaptions(): (tree: Root) => void {
  return (tree) => {
    visit(tree, "element", (node, index, parent) => {
      ...
      let pIndex = index - 1;
      while (pIndex >= 0 && whitespace(parent.children[pIndex])) pIndex--;
      if (pIndex < 0) return;
