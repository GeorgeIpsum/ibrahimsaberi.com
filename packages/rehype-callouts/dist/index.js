...
export default function rehypeCallouts(options) {
    ...
    return (tree) => {
        visit(tree, "element", (node, index, parent) => {
            ...
            let headIndex = 0;
            while (headIndex < node.children.length &&
            ...
            const knownCallout = callouts.get(name) ??
                callouts.get(name.toUpperCase()) ??
                callouts.get(name.toLowerCase());
            if (!knownCallout)
                return;
            // `allowTitle` callouts may carry an inline title on the marker line
            // itself (`> [!EDIT] May 29, 2020`). Capture it, then splice it out of
            // the text so the marker branches below see a bare `[!name]` prefix.
            let titleAddendum;
            if (knownCallout.allowTitle === true &&
                text.value.charAt(end + 1) === " ") {
                const newlineIndex = text.value.indexOf("\n", end + 1);
                const titleEnd = newlineIndex === -1 ? text.value.length : newlineIndex;
                const candidate = text.value.slice(end + 1, titleEnd).trim();
                if (candidate)
                    titleAddendum = candidate;
                text.value = text.value.slice(0, end + 1) + text.value.slice(titleEnd);
            }
            if (end + 1 === text.value.length) {
                ...
                if (next) {
                    ...
                    while (headIndex + 1 < node.children.length &&
                        whitespace(node.children[headIndex + 1])) {
                        skipped = true;
                        headIndex++;
                    }
                    // Exit if there’s no following element.
                    if (headIndex + 1 === node.children.length ||
            ...
            const calloutParagraphChildren = [];
            if (knownCallout.icon) {
                ...
                const svgElement = {
                    ...
                    properties: {
                        ...
                        stroke: knownCallout.icon.strokeOrFill === "stroke"
                            ? "currentColor"
                            : "none",
                        fill: knownCallout.icon.strokeOrFill === "fill"
                            ? "currentColor"
                            : "none",
                        ariaHidden: "true",
                    },
            ...
            if (titleAddendum) {
                titleTextSpans.push({
                    type: "element",
                    tagName: "span",
                    properties: { className: "markdown-callout-title-text-addendum" },
                    children: [{ type: "text", value: titleAddendum }],
                });
            }
                ...
                children: [
                    {
                        ...
                        children: calloutParagraphChildren.concat(titleTextSpans),
                    },
